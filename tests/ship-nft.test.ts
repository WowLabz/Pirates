  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    for (let turn = 0; turn < 100; turn++) {
      ...
      if (!events["nft_mint_event"].asset_identifier.endsWith("pirates")) {
        ship = events["nft_mint_event"];
        break;
      }
    }
    // now i have minted a ship for wallet_1
    ...
    block = chain.mineBlock([
      Tx.contractCall(
        ...
        [types.uint(parseInt(ship.value.slice(1))), types.utf8("combined-rle")],
        wallet_1.address
      ),
    ]);
    block.receipts[0].result.expectOk().expectBool(true);
    ...
    block = chain.mineBlock([
      Tx.contractCall(
        ...
        `combine-traits-to-one`,
        [
          types.uint(parseInt(ship.value.slice(1))),
    ...
    // get get-total-stacked-ships, should be one
    block = chain.mineBlock([
      Tx.contractCall(
        "ship-nft",
        `get-total-stacked-ships`,
        [],
        wallet_1.address
      ),
    ]);
