...
(define-constant OUT_OF_BOUND (err 405))
...
(define-public (claim-trs (token-id uint)) 
    (let
        ...
        (asserts! (is-eq (unwrap-panic (as-contract (contract-call? .trs-token transfer-claimed-trs sender-amount tx-sender))) true) err-trs-transfer-failed)
