...
(define-public (mint (recipient principal))
    (let 
        ...
        (if (<= (var-get total-nft) (var-get stx-nft-count))
            ...
            ;; mint from trs
            (begin 
                (asserts! (<= (var-get minting-trs-amount) (unwrap-panic (contract-call? .trs-token get-balance tx-sender))) ERR-INSUFFICIENT-VALUE)
                (asserts! (is-eq (unwrap-panic (mint-nft recipient)) true) (err u34))
                (asserts! (is-eq (unwrap-panic (contract-call? .trs-token transfer (var-get minting-trs-amount) tx-sender game-recipient none)) true) ERR-MINTING-AMOUNT-TRANSFER-FAILED)
                (var-set total-nft (+ (var-get total-nft) u1))
                (map-set user-buffer tx-sender (+ (get-user-buffer tx-sender) u1))
                ;; (unwrap-panic (tranfer-to-stacked-pirate (- (unwrap-panic (contract-call? .pirate-nft get-last-token-id )) u1)))
                (ok true)
